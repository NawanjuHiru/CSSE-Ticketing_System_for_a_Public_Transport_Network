...
@section('title', 'Route')
@section('sub_title', 'Time Table / ' . $routeData->routeNo . ' ' . $routeData->startPoint . '-' . $routeData->endpoint
    . ' Route')
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card custom-card">
                <div class="card-body">
                    <h6 class="card-title mb-1">Search Time Tables</h6>
                    <form method="POST" class="login-form" id="form_id">
                                ...
                                <div class="form-group @error('day') has-danger @enderror">
                                    ...
                                    <select class="form-control select2" name="day" id="day">
                                        ...
                                        <option value="Monday" label="Monday"
                                            @if (!empty(old('day') && old('day') == 'Monday')) selected @endif>
                                            Monday
                                        </option>
                                        <option value="Tuesday" label="Tuesday"
                                            @if (!empty(old('day') && old('day') == 'Tuesday')) selected @endif>
                                            Tuesday
                                        </option>
                                        <option value="Wednesday" label="Wednesday"
                                            @if (!empty(old('day') && old('day') == 'Wednesday')) selected @endif>
                                            Wednesday
                                        </option>
                                        <option value="Thursday" label="Thursday"
                                            @if (!empty(old('day') && old('day') == 'Thursday')) selected @endif>
                                            Thursday
                                        </option>
                                        <option value="Friday" label="Friday"
                                            @if (!empty(old('day') && old('day') == 'Friday')) selected @endif>
                                            Friday
                                        </option>
                                        <option value="Saturday" label="Saturday"
                                            @if (!empty(old('day') && old('day') == 'Saturday')) selected @endif>
                                            Saturday
                                        </option>
                                        <option value="Sunday" label="Sunday"
                                            @if (!empty(old('day') && old('day') == 'Sunday')) selected @endif>
                                            Sunday
                                        </option>
                                    </select>
                                    @error('day')
                        ...
                        <div class="row card-footer">
                            ...
                            <div class="col-md-6 text-right">
                                <div class="form-group col-md-12">
                                    <a href="{{ route('timetable_view', ['id' => $routeId]) }}" type="button"
                                        class="btn btn-secondary text-white">View Timetable</a>
                                    <a href="{{ route('create_timeTable_view', ['id' => $routeId]) }}" type="button"
                                        id="add" class="btn btn-success">Add Time Table</a>
                                </div>
                            </div>
                        </div>
                    </form>
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.success == 1) {
                                    $('*[data-timetableid="' + timetableid + '"]').closest("tr")
                                        .remove();
                                    Swal.fire(
                                        'Deleted!',
